...
export type SetupFn = (opts: { host: string }) => Promise<void>

export function createUseSharedNuxt(setup: SetupFn): () => Promise<void> {
  return async () => {
    const host = process.env.NUXT_TEST_HOST
    if (!host) {
      throw new Error(
        'NUXT_TEST_HOST не выставлен — vitest globalSetup / fork-init / playwright fixture должен поднять test-stack первым',
      )
    }
    await setup({ host })
  }
