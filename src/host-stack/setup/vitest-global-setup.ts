...
export function createVitestGlobalSetup(ctx: HostStackContext): () => Promise<() => Promise<void>> {
  return async () => {
    ...
    console.log(
      `[test-stack] connected ${String(res.workerCount)} worker(s) migrations=${res.migrationSkipped ? 'skipped' : 'applied'} hosts=${res.hosts.slice(1).join(',')}`,
    )
