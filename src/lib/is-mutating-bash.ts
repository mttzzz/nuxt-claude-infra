...
export interface MutatingVerdict {
  mutating: boolean
  reason: string | null
}

export function isMutatingBash(command: string): MutatingVerdict {
...
function pushIfNotEmpty(list: string[], part: string): void {
...
function classifySubCommand(sub: string): MutatingVerdict {
...
function classifyGit(args: string[]): MutatingVerdict {
...
function classifyBun(bin: string, args: string[]): MutatingVerdict {
...
function classifyDocker(args: string[]): MutatingVerdict {
  const sub = findFirstNonFlag(args)
  if (!sub) {
    return { mutating: false, reason: null }
  }
...
function classifyNpmLike(args: string[]): MutatingVerdict {
  const sub = findFirstNonFlag(args)
  if (!sub) {
    return { mutating: false, reason: null }
  }
...
function classifyPrisma(args: string[]): MutatingVerdict {
  const sub = findFirstNonFlag(args)
  if (!sub) {
    return { mutating: false, reason: null }
  }
...
function tokenize(sub: string): CommandToken | null {
...
function findFirstNonFlag(args: string[]): string | null {
  for (const a of args) {
    if (!a.startsWith('-')) {
      return a
    }
