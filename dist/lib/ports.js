...
function harnessFile(harnessPid) {
    return join(SESSIONS_BY_HARNESS_DIR, `${String(harnessPid)}.json`);
}
export function writeSessionByHarness(harnessPid, sessionId) {
    if (!harnessPid || harnessPid <= 0 || !sessionId)
        return;
    mkdirSync(SESSIONS_BY_HARNESS_DIR, { recursive: true });
    const data = { sessionId, harnessPid, writtenAt: Date.now() };
    writeFileSync(harnessFile(harnessPid), JSON.stringify(data), 'utf8');
}
export function readSessionByHarness(harnessPid) {
...
export function removeSessionByHarness(harnessPid) {
    try {
 ...
 */
export function pruneStaleHarnessFiles(isPidAliveFn = isPidAlive) {
...
export function slugSessionId(sessionId) {
...
function portsFile(sessionId) {
    return join(sessionDir(sessionId), 'ports.json');
}
export function getSessionPorts(sessionId) {
...
export function freeSessionPorts(sessionId) {
    try {
...
async function withAllocLock(fn) {
...
export async function isPortFree(port) {
    ...
    if (await isAccepting(port))
        return false;
    return isBindable(port);
}
async function findFirstFree(role, excluded) {
...
export async function allocateSessionPorts(sessionId) {
