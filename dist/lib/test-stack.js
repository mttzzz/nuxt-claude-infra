...
import { allocateSessionPorts, freeSessionPorts, resolveSessionId, SESSIONS_DIR, } from './ports.js';
import { buildComposeEnv, buildTestServerImage, composeProjectName, startTestStackContainers, stopTestStackContainers, } from './docker.js';
 ...
 */
export async function allocateWithRetry(sessionId, opts) {
    let lastError;
...
function handleFilePath(sessionId) {
 ...
 */
export async function startTestStack(deps = {}) {
    ...
    const handle = { sessionId, host, ports, composeProjectName: projectName };
 ...
 */
export async function stopTestStack(handleOrSessionId, deps = {}) {
    const sessionId = typeof handleOrSessionId === 'string' ? handleOrSessionId : handleOrSessionId.sessionId;
 ...
 *   await stack.stop()
 */
export function defineTestStack(deps = {}) {
    let handle = null;
    return {
        async start() {
 ...
 */
async function waitForReady(host, timeoutMs) {
