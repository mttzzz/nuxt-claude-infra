...
export function hashBuildInputs(rootDir, dirs, files) {
...
export function hashMigrations(rootDir) {
    ...
    for (const dir of dirs) {
        ...
        if (existsSync(snap))
            hash.update(readFileSync(snap));
    }
    return hash.digest('hex').slice(0, 8);
}
export function loadCachedHash(path) {
    if (!existsSync(path))
        return null;
    try {
...
export async function ensureTestDb(opts) {
...
export function runMigrationsIfNeeded(opts) {
 ...
 */
export async function ensureTestStack(ctx, opts = {}) {
